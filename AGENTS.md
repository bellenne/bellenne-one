...
Перед задачами, затрагивающими Folio, прочитай `docs/products/FOLIO.md` и `docs/TZ_FOLIO.md`. Перед любым UI-изменением дополнительно прочитай `docs/BRANDBOOK.md`. Перед изменением API Ozon проверь актуальную документацию, права аккаунта, доступность методов и существующий код интеграции. Если файл отсутствует — сообщи об этом, не подменяй его догадкой.

## 2. Границы ответственности

Folio принимает и подготавливает заказы на картины по фотографиям, ведёт покупателя по настроенному сценарию через поддерживаемые чаты Ozon и формирует структурированный бриф. Folio **не** редактирует изображения, не рисует картины, не подменяет CRM, не запускает производство без отдельной явно реализованной интеграции. Не изменяй Pulse, Echo, Vector, Nest, Proof и общий Bellenne Shell ради реализации Folio без прямого требования пользователя.

## 3. Продуктовые типы

Ровно три стартовых типа товара:
- `portrait_background` — обычная картина с заменой фона;
- `collage` — коллаж максимум из 8 фотографий с текстом;
- `template_art` — сильная обработка по одному шаблону из доступного каталога.

Соответствие Ozon SKU/offer_id конкретному типу задаётся администратором. Не определяй тип товара по похожести названия, не придумывай четвёртый тип, не назначай шаблон автоматически. Если один заказ/отправление содержит несколько товарных позиций, не смешивай данные разных позиций.

## 4. Bot-builder — обязательный визуальный редактор

**Главный способ редактирования сценария — пользовательский node-based canvas, а не список полей, JSON или редактор кода.** Администратор видит шаги и связи на схеме, создаёт узлы из ограниченной палитры, соединяет их и редактирует выбранный узел в правом инспекторе. Начальное событие «входящий заказ/сообщение», тексты ответов, ожидаемое сообщение клиента, сбор фото, условия, переходы, передача менеджеру и завершение настраиваются через обычные элементы UI. Тип узла и его возможности должны соответствовать реально поддерживаемому движку и подтверждённому каналу Ozon.

Обязательно: наглядная последовательность шагов; названия узлов; видимые подписанные переходы; создание/перенос/удаление узлов и связей; zoom/pan; сохранение схемы и координат; редактирование текста сообщения с разрешёнными переменными и предпросмотром; несохранённые изменения; кнопки «Проверить», «Тестировать без отправки» и «Опубликовать». Публикация блокируется при ошибках графа; точные ошибки показываются на конкретном узле и в списке проблем. Не требовать от администратора редактировать JSON или программировать. Для доступности и длинных схем предоставить синхронизированное представление «Список шагов», но **не заменять им визуальный граф**.

Этапы, сообщения, проверки, условия и переходы берутся из опубликованной версии сценария в БД. Не хардкодь тексты/граф в обработчиках и frontend. Черновик не влияет на активные диалоги: они привязаны к версии. Новая публикация применяется новым экземплярам; скрытую миграцию запрещено выполнять. Не добавляй неподдерживаемые узлы или переходы «на будущее».

## 5. Защита от ошибок автоматизации

- Синхронизация Ozon может использовать только реально доступные API/события; не предполагать наличие webhooks чатов или заказов без проверки.
- Входящие события и сообщения обрабатывать идемпотентно по реальным внешним идентификаторам и собственным ключам. Не терять законные повторные заказы.
- На один чат/контекст обработки обеспечить последовательное применение событий: блокировка/версия состояния, отсутствие двойного перехода по одному сообщению.
- Ручной ответ менеджера и бот не должны отправлять конфликтующие сообщения: механизм takeover/pause должен атомарно запрещать новые автоответы и проверяться непосредственно перед отправкой.
- У каждого исходящего сообщения отслеживать `pending / sent / failed / unknown` либо эквивалентные реально реализованные состояния. `unknown` не означает, что сообщение не отправлено: не повторять автоматически без сверки.
- Не отправлять сообщения или файлы от имени продавца, если нет подтверждённых прав и поддерживаемого API.
- Не генерировать содержательные обещания о сроках, качестве, скидках, готовности или действиях с заказом, если они не определены в утверждённом тексте/данных.

## 6. Права и безопасность

Роли **только `admin` и `manager`** в стартовой версии. `admin`: управление пользователями, настройками Ozon, привязкой товаров, сценариями, шаблонами, журналами и чатами. `manager`: исключительно интерфейс чатов, просмотр необходимого контекста текущего заказа, вложений и переписки, ручные ответы, передача конкретного чата в ручной режим/возврат боту в пределах утверждённой политики. `manager` не получает доступ к редактору сценариев, настройкам интеграций, токенам, пользователям, общему audit log или отдельному разделу заказов. Проверять доступ **на backend** для каждого метода, не только скрывать кнопки.

Фотографии заказчиков и переписка — приватные данные: защищённое хранилище, ограниченный доступ, секреты только на сервере, маскировка в логах, проверка типов/размеров файлов, защита от обхода путей, безопасная обработка внешних ссылок. Не задавать произвольные сроки хранения без решения пользователя.

## 7. UI и Bellenne

Использовать Bellenne Shell, существующие компоненты и `docs/BRANDBOOK.md`. Folio — **visual workflow builder + чистый messenger-style inbox**, не BI-dashboard и не графический редактор изображений. Новый Product Accent не назначать самостоятельно: использовать существующие токены до утверждения. Не переделывать Pulse и другие продукты.

**Чаты:** две постоянные зоны — компактный список слева и просторная переписка справа; карточка заказа/прогресс бота открывается как контекстный drawer по кнопке, а не загромождает окно обязательной третьей колонкой. В переписке: удобные сообщения/вложения, заметные статусы отправки, понятный режим `bot/manual`, нижний composer. Никакой технической телеметрии, полного аудита, секретов и формы настройки Ozon в рабочем чате. У manager в навигации только «Чаты» и необходимые ему действия.

**Настройка не в одной форме:** admin получает отдельные разделы «Ozon», «Товары», «Сценарии», «Проверка и запуск», «Журнал», «Пользователи». Первичное подключение — мастер шагов: (1) Client ID и API-ключ, (2) отдельная проверка соединения и прав чатов/медиа/заказов, (3) сопоставление SKU с 3 типами товаров, (4) выбор/публикация сценариев, (5) тестовый прогон без отправки, (6) активация после проверок. Можно сохранить незавершённую настройку и вернуться к шагу. Каждая страница содержит только тематические настройки. Данные ключа в UI маскировать, на сервере хранить защищённо, никогда не отдавать обратно в открытом виде. Проверка «ключ верен» не равна «чаты доступны»: показывать результаты отдельно и не имитировать поддержку API.

**Понятные ошибки:** пользователь видит причину, затронутую сущность и действие для исправления (например, неверный ключ, нет доступа к чатам, неверный SKU mapping, отсутствует ответная ветка, вложение недоступно); admin может открыть безопасные технические подробности с correlation/request ID. При неопределённой отправке не показывать «не отправлено» и не предлагать слепой повтор. Новые UI-зависимости допустимы только после осмотра стека и обоснования: совместимый node editor может понадобиться, но не подключать несколько UI kits.

## 8. Реализация и контроль качества

Сначала изучи существующий репозиторий и составь краткую карту текущих модулей/ограничений. Затем реализуй минимальный вертикальный срез с проверками: получение заказа/сообщения → определение типа → сценарий → чат → данные брифа → ручная проверка → готовность. Внешний API изолируй в адаптере, события обрабатывай через очередь и транзакционные границы, не держи тяжёлую работу внутри HTTP-запросов. Новые зависимости — только при документированной потребности.

Обязательные тесты: визуальное редактирование узлов и связей, сохранение/публикация/симуляция, мастер Ozon с маскировкой ключа и отдельной проверкой прав, чистый Inbox и контекстный drawer, понятные ошибки; RBAC; три типа товаров; коллаж `> 8` фото; отсутствие обязательного шаблона; неверные/повторные события; одновременный бот и менеджер; ошибка/неопределённый результат отправки; смена версии сценария; отсутствие доступа к API; передача в производство только после явно определённой готовности. Не помечай задачу завершённой, пока тесты не выполнены или ограничения честно не описаны.

## 9. Приоритет и неясности

Текущая явная команда пользователя → безопасность и фактические контракты → настоящий `AGENTS.md` → `docs/TZ_FOLIO.md` (поведение) → `docs/products/FOLIO.md` (продукт и UX) → `docs/BRANDBOOK.md` (визуал) → существующие компоненты. При конфликте объясни конфликт и запроси решение; не выдумывай функциональность. В конце каждой задачи кратко перечисли изменения, тесты, неготовые зависимости и вопросы.
